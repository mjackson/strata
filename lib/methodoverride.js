    ...
    return function methodOverride(env, callback) {
        if (env.requestMethod === "POST") {
            ...
            req.params(function (err, params) {
                ...
                if (method) {
                    env["strata.methodOverride.originalMethod"] = env.requestMethod;
                    env.requestMethod = method.toUpperCase();
                }

                app(env, function (status, headers, body) {
                    if (env["strata.methodOverride.originalMethod"]) {
                        env.requestMethod = env["strata.methodOverride.originalMethod"];
                        delete env["strata.methodOverride.originalMethod"];
                    }

...
function checkEnv(env) {
    ...
    // - strata.version must be an array of integers [major, minor, patch]
    assert(Array.isArray(env["strata.version"]), "strata.version must be an array");
    assert(env["strata.version"].length == 3, "strata.version must contain three values (major, minor, patch)");
    env["strata.version"].forEach(function (n, i) {
        assert(typeof n == "number", "Index " + i + " of strata.version must be a number");
    });

    // - strata.input must be an EventEmitter of data contained in the request body
    assert(env["strata.input"] instanceof EventEmitter, "strata.input must be an EventEmitter");

    // - strata.error must be a writable Stream
    assert(env["strata.error"] instanceof EventEmitter, "strata.error must be a Stream");
    assert(env["strata.error"].writable, "strata.error must be writable");

    // - strata.session      A JavaScript object containing session data.
    var session = env["strata.session"];
    if (session) {
        assert(typeof env == "object", "strata.session must be an object");
    }
}

 ...
 */
Response.prototype.__defineGetter__('empty', function () {
  if (this.body === '') {
    return true;
  }

  return utils.isEmptyBodyStatus(this.status);
});

/**
 * Returns true if this response uses chunked transfer encoding.
 */
Response.prototype.__defineGetter__('chunked', function () {
 ...
 */
Response.prototype.hasHeader = function (name) {
  name = utils.canonicalHeaderName(name);
 ...
 * in the response.
 */
Response.prototype.removeCookie = function (name, value) {
  value = value || {};
...
// Define getters and setters for all response headers using the camel-cased
// version of the header name on Response.prototype, e.g.:
//
// Host => host

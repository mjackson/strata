...
function Parser(boundary, tmpdir, prefix) {
    ...
    if (path.existsSync(this.tmpdir)) {
        ...
        if (!stats.isDirectory()) {
            throw new errors.Error('"' + this.tmpdir + '" is not a directory')
        }
    } else {
        throw new errors.Error('Directory "' + this.tmpdir + '" does not exist');
    }
...
Parser.prototype.end = function end() {
    if (this.state !== S.END) {
        throw new errors.Error("Stream ended unexpectedly (state: " + this.state + ")");
    }

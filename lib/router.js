 ...
 * restricting matches to a specific request method. Populates the "strata.route"
 * environment variable with an object containing the results of the match.
 * Tries all routes in the order they were defined.
 */
function Router(app) {
    this.app = app || utils.notFound;
    this.routes = {};
}
...
function tryRoutes(routes, env, callback) {
    ...
    for (var i = 0, len = routes.length; i < len; ++i) {
        ...
        if (match) {
            ...
            var oldRoute = env["strata.route"];
            env["strata.route"] = obj;

            app(env, function (status, headers, body) {
                // Restore the old value of strata.route for upstream apps.
                if (oldRoute) {
                    env["strata.route"] = oldRoute;
                } else {
                    delete env["strata.route"];
                }

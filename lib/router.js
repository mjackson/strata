 ...
 * restricting matches to a specific request method. Populates the "link.route"
...
function tryRoutes(routes, env, callback) {
    ...
    for (var i = 0, len = routes.length; i < len; ++i) {
        ...
        if (match) {
            ...
            var oldRoute = env["link.route"];
            env["link.route"] = obj;

            app(env, function (status, headers, body) {
                // Restore the old value of link.route for upstream apps.
                if (oldRoute) {
                    env["link.route"] = oldRoute;
                } else {
                    delete env["link.route"];
                }

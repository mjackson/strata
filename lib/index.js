...
function makeEnv(opts) {
    ...
    if (!(opts.input instanceof EventEmitter)) {
        throw new StrataError("Input must be an EventEmitter");
    }
    ...
    env["strata.version"] = exports.version;
    env["strata.input"] = new Input(opts.input);
    env["strata.error"] = opts.error || process.stderr;
...
/**
 * The default error handler simply logs the error stack to strata.error and
...
function handleError(error, env, callback) {
    env["strata.error"].write("Unhandled error!\n" + error.fullStack);

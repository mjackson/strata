 ...
 *                  defaults to null. If this is set the session's contents will
 *                  be cleared if the cookie has been tampered with.
 *   - name         The name of the cookie, defaults to "link.session"
    ...
    return function writeSessionCookie(env, res) {
        ...
        if (session) {
            ...
            if (cookie.length > 4096) {
                env["link.error"].write("Session cookie data size exceeds 4k; content dropped\n");
                return;
            }
...
function sessionCookieOptions(options) {
    ...
    var opts = {
        ...
        name: options.name || "link.session",
        path: options.path || "/",

...
function Parser(boundary, tmpdir, prefix) {
    ...
    if (path.existsSync(this.tmpdir)) {
        ...
        if (!stats.isDirectory()) {
            throw new link.Error('"' + this.tmpdir + '" is not a directory')
        }
    } else {
        throw new link.Error('Directory "' + this.tmpdir + '" does not exist');
    }
...
Parser.prototype.end = function end() {
    if (this.state !== S.END) {
        throw new link.Error("Stream ended unexpectedly (state: " + this.state + ")");
    }
...
File.prototype.unlink = function unlink(callback) {

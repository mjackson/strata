...
Request.prototype.cookies = function (callback) {
  if (!this.env.headers['cookie']) {
    callback(null, {});
    return;
  }

  if (this.env.strataCookieString !== this.env.headers['cookie']) {
    this.env.strataCookieString = this.env.headers['cookie'];
  ...
  parser.write(new Buffer(this.env.headers['cookie']));
  parser.end();
};

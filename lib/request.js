...
Request.prototype.cookies = function (callback) {
  var cookieString = this.env.headers['cookie'];
  ...
  if (this.env.strataCookieString !== cookieString) {
    this.env.strataCookieString = cookieString;
  ...
  this.env.strataCookies = cookies;
...
Request.prototype.query = function (callback) {
  var queryString = this.env.queryString;
  ...
  if (this.env.strataQueryString !== queryString) {
    this.env.strataQueryString = queryString;
  ...
  if (this.mediaType === 'application/json') {
    ...
    input.on('end', function () {
      try {
        ...
        var err = new strata.Error('Invalid JSON', e);
        callback(err, {});
      }
    });
    ...
    if (match) {
      ...
      try {
        ...
        callback(new strata.Error('Cannot create multipart parser', e), body);
        return;
      }
      ...
      input.on('data', function (buffer) {
        ...
        if (length !== buffer.length) {
          input.pause();
          callback(new strata.Error('Error parsing multipart body: ' + length + ' of ' + buffer.length + ' bytes parsed'), body);
        }
      });
      ...
      input.on('end', function () {
        try {
          ...
          callback(new strata.Error('Error parsing multipart body', e), body);
        }
      });
    } else {
      var buffer = '';

    ...
    if (contentType === "application/json") {
        ...
        if (match) {
            ...
            try {
                ...
                callback(new link.Error("Cannot create multipart parser", e), params);
                return;
            }
            ...
            input.on("data", function (buffer) {
                ...
                if (length !== buffer.length) {
                    input.pause();
                    callback(new link.Error("Error parsing multipart body: " + length + " of " + buffer.length + " bytes parsed"), params);
                }
            });
            ...
            input.on("end", function () {
                try {
                    ...
                    callback(new link.Error("Error parsing multipart body", e), params);
                }
            });
        } else {
...
Request.prototype.params = function params(callback) {
    if (this.env["link.request.params"]) {
        return callback(null, this.env["link.request.params"]);
    }
